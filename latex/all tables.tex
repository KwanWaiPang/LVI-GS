...
\clearpage % 强制换页，使表格出现在新的一页
\begin{table*}[!p] % 使用 !p 让表格独占一页
    \centering
    \vfill % 使表格在页面垂直方向上居中
    \caption{Summarization of different NeRF-based SLAM and 3DGS-based SLAM.}
    \renewcommand{\arraystretch}{1.0} % 调整行间距
    \setlength{\tabcolsep}{4pt} % 调整列间距
    \resizebox{\linewidth}{!}{ % 调整表格大小适应页面宽度
    \begin{tabular}{@{}m{2.5cm}m{2.5cm}m{5cm}m{5cm}@{}}
    \toprule
    \textbf{Category} & \textbf{Method Type} & \textbf{Typical Methods} & \textbf{Features} \\
    \midrule
    \textbf{NeRF-based SLAM} 
    & Image based 
    & iMAP\cite{imap}; NICE-SLAM\cite{niceslam}; Vox-Fusion~\cite{voxfusion}; ESLAM\cite{esslam}; Co-SLAM\cite{coslam}; NeRF-SLAM\cite{nerfslam}; NeRF-VO\cite{nerfvo} 
    & - Strong capability for novel view synthesis \newline
    - Low rendering efficiency \newline
    - Expensive training cost \\
    \midrule
    \multirow{2}{=}{\rule{0pt}{6ex} \textbf{3DGS-based SLAM}} 
    & Image based 
    & MonoGS\cite{monogs}; Photo-SLAM\cite{photoslam}; SplaTAM\cite{splatam}; GS-SLAM\cite{gsslam}; GS-ICP\cite{gsicp} 
    & - Fast rendering and high fidelity\newline
    - Fast training speed \newline
    - Noisy initialization \\
    \cmidrule(lr){2-4}
    & LiDAR-Image based 
    & Gaussian-LIC\cite{gaussianlic}; MM-Gaussian\cite{mmgaussian}; MM3DGS-SLAM\cite{mm3dgs} 
    & - Fast rendering and high fidelity\newline
...
\clearpage % 强制换页，使该图像独立占据一页
\begin{figure*}[!p] % 使用 !p 让图像出现在独立的浮动页面
    \centering
    \vfill % 使图片在页面中垂直居中
    % 子图 (a)
...
\clearpage % 强制换页，使该表格独立占据一页
\begin{table*}[!p] % 使用 !p 让表格放置在独立的浮动页面
    \centering
    \vfill % 使表格在页面中垂直居中
...
\begin{table*}[!p] % 让表格出现在单独的浮动页面
    ...
    \resizebox{0.8\linewidth}{!}{ % 调整表格宽度以适应页面
    \begin{tabular}{lcccc}
        \toprule
        \textbf{Configuration} & \textbf{PSNR}$\uparrow$ & \textbf{SSIM}$\uparrow$ & \textbf{LPIPS}$\downarrow$ & \textbf{Time (s)} \\
        \midrule
        w/ Keyframe selection and delay         & \textbf{22.36} & \textbf{0.747} & \textbf{0.269} & 100 \\
...
\begin{table*}[!p] % 让表格出现在单独的浮动页面
    \centering
    \vfill % 让表格在页面中垂直居中
    \caption{Time consumption of several key components of our framework.}
    \renewcommand{\arraystretch}{1.2} % 增加行间距，提高可读性
    \setlength{\tabcolsep}{12pt}      % 调整列间距
    \resizebox{0.6\linewidth}{!}{ % 适应页面宽度
    \begin{tabular}{l r}
        \toprule
        \textbf{Component}                & \textbf{Time Cost (ms)} \\ 
        \midrule
